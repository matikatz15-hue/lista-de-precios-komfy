...
const NAV = [
  ...
  { href: "/admin/lineas", label: "Líneas", icon: "≡" },
  { href: "/admin/grupos", label: "Grupos", icon: "⊞" },
  { href: "/admin/productos", label: "Productos", icon: "◈" },
  { href: "/admin/condiciones", label: "Condiciones", icon: "%" },
...
export default function AdminSidebar() {
  ...
  return (
    <aside className="w-56 bg-[#0047BB] text-white flex flex-col shrink-0">
      <div className="px-5 py-6 border-b border-white/10">
        <svg viewBox="0 0 332 100" xmlns="http://www.w3.org/2000/svg" fill="white" className="h-6">
        ...
        <p className="text-xs text-white/50 mt-1 font-medium tracking-widest uppercase">Admin</p>
      </div>
      ...
      <div className="px-3 py-4 border-t border-white/10 space-y-2">
        <Link href="/" target="_blank" className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-white/65 hover:text-white hover:bg-white/10 transition-colors">
          <span>↗</span> Ver catálogo A4
        </Link>
        <Link href="/mobile" target="_blank" className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-white/65 hover:text-white hover:bg-white/10 transition-colors">
          <span>↗</span> Ver catálogo mobile
        </Link>
        <button
          onClick={() => signOut({ callbackUrl: "/login" })}
          className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-white/65 hover:text-white hover:bg-white/10 transition-colors text-left"
        >
